...

{
    ...
    public function lastVowel($word)
    {
        ...
        foreach ($letters as $letter) {
            ...
            } elseif ($this->contains($this->BACK_VOWELS, $letter)) {
                ++$vowel_count;
        ...
        if (in_array($word, $this->EXCEPTION_WORDS)) {
            ...
            } elseif ($returndata['letter'] == 'a') {
                $returndata = ['letter' => 'e', 'tone' => 'back'];
            } elseif ($returndata['letter'] == 'u') {
    ...
    public function lastLetter($word)
    {
        ...
        if ($getLastLetter == "'") {
            $getLastLetter = substr($word, strlen($word) - 2)[0];
        }  // charAt
    ...
    public function makeInfinitive($word)
    {
    ...
    public function makeDerivative($word, $param)
    {
        ...
        return $val['tone'] == 'front' ? $this->concat($word, 'lı') : $this->concat($word, 'li');
    }

    public function makePlural($word, $param)
    {
        if ($param['proper_noun']) {
            $word .= "'";
        }
    ...
    public function makeAccusative($word, $param)
    {
        ...
        if ($lowerWord == 'o') {
            ...
            } elseif ($getLastLetter['discontinious_hard_consonant_for_suffix'] == true && $param['proper_noun'] == false) {
    ...
    public function makeDative($word, $param)
    {

        //firslty exceptions for ben (I) and you (sen)
        //$returndata; //Todo check
        $lowerWord = $this->makeLower($word);
        ...
        if ($lowerWord == 'ben' && $param['proper_noun'] == false) {
        ...
        } elseif ($lowerWord == 'sen' && $param['proper_noun'] == false) {
            ...
            if ($getLastLetter['vowel'] == true) {
                $word = $this->concat($word, 'y');
            } elseif ($getLastLetter['discontinious_hard_consonant_for_suffix'] == true) {
    ...
    public function makeGenitive($word, $param)
    {
        $getLastLetter = $this->lastLetter($word);
        $getLastVowel = $this->lastVowel($word);
        $lowerWord = $this->makeLower($word);
    ...
    public function makeAblative($word, $param)
    {
        $getLastLetter = $this->lastLetter($word);
        $getLastVowel = $this->lastVowel($word);
    ...
    public function makeLocative($word, $param)
    {
        $getLastLetter = $this->lastLetter($word);
        $getLastVowel = $this->lastVowel($word);
    ...
    // İyelik ekleri
    public function possessiveAffix($word, $param)
    {
        $person = $param['person'];
        ...
        if ($person != '3' && $quantity != 'plural') { // Todo
            ...
            if (isset($param['proper_noun']) && $param['proper_noun']) {
                $word .= "'";
            } elseif ($getLastLetter['discontinious_hard_consonant_for_suffix']) {
                if ($getLastVowel['vowel_count'] > 1) {
                    $word = $this->concat(substr($word, 0, strlen($word) - 1), $getLastLetter['soften_consonant_for_suffix']);
                }
    ...
    public function makeLower($word)
    {
        $word = str_replace('İ', 'i', $word);
    ...
    public function makeUpper($word)
    {
    ...
    public function concat($word, $str)
    {
        return $this->isUpper($word) ? $word.$this->makeUpper($str) : $word.$str;
    }

    public function fromUpperOrLower($newWord, $refWord)
    {
        $returndata = null;

        if ($this->isUpper(substr($refWord, strlen($refWord) - 1))) {
            $returndata = $this->makeUpper($newWord);
        } else {
            if ($this->isUpper(substr($refWord, 0))) { // Todo
                $returndata = $this->makeUpper(substr($newWord, 0)).$this->makeLower(substr($newWord, 1, strlen($newWord) - 1));
            }  // Todo
            else {
